...
import type { StoredImage } from "@/lib/battleQueries";
import { getObject, r2Configured } from "@/lib/r2";

/*
 * 보관함 사진을 내주는 공통 길. 자리 사진과 거점 지도가 같이 쓴다.
 *
...
export async function storedImageResponse(
  ...
  return new Response(new Uint8Array(object.body), {
    headers: {
      ...
      /* 주소는 자리·거점마다 하나뿐이라 사진을 바꿔도 그대로다. 오래 물고 있게
         두면 새 사진이 안 보이므로, 열쇠를 꼬리표로 주고 매번 물어보게 한다. */
      ETag: tag,
